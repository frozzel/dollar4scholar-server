...
exports.addDonorContribution = async (req, res) => {
        ...
        if (req.body.amount > user.wallet) return sendError(res, 'Insufficient funds!', 400)
        ...
        scholarship.pot += req.body.amount; // add the amount to the scholarship pot
        await scholarship.save();
...
exports.addMoneyToPot = async (req, res) => {
    try {
        ...
        scholarship.pot += req.body.amount;
        await scholarship.save();
        res.status(200).json({ message: 'Money added to pot successfully', scholarship });
    } catch (error) {
        res.status(400).json({ message: 'Error adding money to pot', error });
    }
};
